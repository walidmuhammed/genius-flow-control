...
-- 1. Ensure settlement_id is properly unique and sequential
DROP TRIGGER IF EXISTS set_settlement_id ON courier_settlements;
DROP FUNCTION IF EXISTS generate_settlement_id;
DROP FUNCTION IF EXISTS set_settlement_id;
...
-- Create the trigger
CREATE TRIGGER set_settlement_id
    BEFORE INSERT ON public.courier_settlements
    FOR EACH ROW
    EXECUTE FUNCTION public.set_settlement_id();

-- 2. Add function to automatically calculate courier fees when order status changes
...
BEGIN
    ...
    IF NEW.status IN ('Delivered', 'Unsuccessful') 
       ...
       AND (NEW.courier_fee_usd IS NULL OR NEW.courier_fee_lbp IS NULL) THEN
        ...
        -- Calculate courier fee using the pricing function
...
-- 3. Add function to create client payouts when settlement is marked paid
...
-- 4. Backfill courier fees for existing delivered/unsuccessful orders
...
-- 5. Ensure courier_settlement_status enum values are consistent
UPDATE public.orders 
SET courier_settlement_status = 'None'
WHERE courier_settlement_status IS NULL;

-- 6. Ensure cash_handover_status enum values are consistent  
UPDATE public.orders 
