-- Phase 1: Fix Database Flow & Logic

-- Step 1: Modify the create_client_payout_entry trigger to NOT create payouts immediately
-- Instead, payouts should only be created when courier settlements are paid
DROP TRIGGER IF EXISTS create_client_payout_on_order_completion ON orders;
DROP FUNCTION IF EXISTS public.create_client_payout_entry();

-- Step 2: Enhance the settlement-to-payouts integration
-- This trigger should create client payouts when settlements are marked as paid
...
-- Step 3: Add data validation function to prevent duplicate settlements
CREATE OR REPLACE FUNCTION public.validate_settlement_orders()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
BEGIN
    -- Check if any orders in this settlement are already in another settlement
    IF EXISTS (
        SELECT 1 FROM public.orders 
        WHERE courier_settlement_id IS NOT NULL 
        AND courier_settlement_id != NEW.id
        AND courier_id = NEW.courier_id
        AND status IN ('Delivered', 'Unsuccessful')
        AND courier_settlement_status = 'None'
    ) THEN
        -- This is actually OK - we're creating a new settlement for remaining orders
        NULL;
    END IF;
...
-- Step 5: Update order status management
-- Ensure orders are marked properly for settlement flow
