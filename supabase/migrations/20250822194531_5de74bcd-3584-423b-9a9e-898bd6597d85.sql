-- Phase 1: Add 'paid' status to orders with all existing statuses
ALTER TABLE public.orders DROP CONSTRAINT IF EXISTS orders_status_check;
ALTER TABLE public.orders ADD CONSTRAINT orders_status_check CHECK (status IN ('New', 'Assigned', 'In Progress', 'Successful', 'Unsuccessful', 'On Hold', 'Pending Pickup', 'Returned', 'paid'));
