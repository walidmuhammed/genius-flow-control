-- Fix courier settlement status handling - first update constraint, then fix data
...
-- Update the check constraint to allow 'Pending Settlement' value
...
ALTER TABLE public.orders ADD CONSTRAINT orders_courier_settlement_status_check 
    CHECK (courier_settlement_status IN ('None', 'Pending Settlement', 'Settled'));
