    ...
    if (hasChildren && !collapsed) {
      return (
        <div key={item.href} className="mb-1">
          <Collapsible 
            open={isGroupOpen}
            onOpenChange={() => toggleGroup(item.label.toLowerCase())}
          >
            <CollapsibleTrigger className="w-full">
              ...
              >
                ...
                <div className="flex items-center gap-1.5">
                  {item.badge && (
                    <span className={cn(
                      "flex min-w-[18px] h-[18px] rounded-full text-[10px] items-center justify-center px-1 font-medium",
                ...
                return (
                  ...
                  >
                    ...
                    {childItem.badge && (
                      <span className={cn(
                        "flex min-w-[18px] h-[18px] rounded-full text-[10px] items-center justify-center px-1 font-medium",
                        getBadgeStyles(childItem.badge.variant)
                      )}>
    ...
    <div className="space-y-1">
      {menuItems.map((item, index) => {
        if ('type' in item && item.type === 'separator' && !collapsed) {
          return (
            <div key={`separator-${index}`} className="mt-4 mb-2 px-3">
