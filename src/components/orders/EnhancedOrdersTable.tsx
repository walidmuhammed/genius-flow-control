...
export const EnhancedOrdersTable: React.FC<EnhancedOrdersTableProps> = ({
  ...
  const [deleteDialogOrder, setDeleteDialogOrder] = useState<OrderWithCustomer | null>(null);
  ...
  const handleEdit = (order: OrderWithCustomer, e: React.MouseEvent) => {
    e.stopPropagation();
    navigate(`/orders/${order.id}/edit`);
  };

  const handleDelete = (order: OrderWithCustomer, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const confirmDelete = async () => {
    ...
    try {
      await archiveOrderMutation.mutateAsync(deleteDialogOrder.id);
  ...
  return (
    <>
      ...
      <AlertDialog open={!!deleteDialogOrder} onOpenChange={() => setDeleteDialogOrder(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
