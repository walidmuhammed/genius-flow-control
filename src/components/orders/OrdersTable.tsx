...
interface OrdersTableProps {
  ...
  selectionMode?: boolean;
  selectedOrderIds?: string[];
  singleSelectRowClickOnly?: boolean; // NEW PROP
  fitModalWidth?: boolean; // NEW PROP
}
...
export const OrdersTable: React.FC<OrdersTableProps> = ({ 
  ...
  selectionMode = false,
  selectedOrderIds = [],
  singleSelectRowClickOnly = false,
  fitModalWidth = false,
}) => {
  ...
  if (singleSelectRowClickOnly) {
    // Only show essential columns, make row clickable, no checkboxes, no actions, fit width
    return (
      <div className={cn(
        "bg-white dark:bg-gray-800 border border-gray-200/50 dark:border-gray-700/30 rounded-2xl overflow-hidden shadow-sm",
        fitModalWidth && "overflow-x-auto"
      )}>
        <Table className={fitModalWidth ? "min-w-full" : undefined}>
          <TableHeader>
            <TableRow className="bg-gray-50/50 dark:bg-gray-800/50 border-b border-gray-200/30 dark:border-gray-700/30 hover:bg-gray-50/50 dark:hover:bg-gray-800/50">
              <TableHead className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider pl-4">ORDER ID</TableHead>
          ...
          <TableBody>
            {orders.map((order) => (
              ...
              >
                <TableCell className="pl-4 font-semibold text-[#DB271E]">
