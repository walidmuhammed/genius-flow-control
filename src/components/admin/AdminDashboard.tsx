...
import { Package, Truck, Clock, CheckCircle, AlertCircle, Users, TrendingUp, Activity, UserPlus, Calendar, Ticket, XCircle, DollarSign, MapPin, Phone, Building2, ArrowUpRight, ArrowDownRight, Star, Zap, Eye } from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  const filteredTickets = tickets.filter(ticket => new Date(ticket.created_at) >= startDate);
  const filteredPickups = pickups.filter(pickup => new Date(pickup.created_at) >= startDate);

  // Calculate metrics
  const failedOrders = orders.filter(order => order.status === 'Unsuccessful' || order.status === 'Returned');
  ...
  const openTickets = tickets.filter(ticket => ticket.status === 'New' || ticket.status === 'Processing').length;
  const unassignedPickups = pickups.filter(pickup => pickup.status === 'Scheduled' && !pickup.courier_name).length;
  ...
  const recentActivity = [...filteredOrders.slice(0, 5).map(order => ({
    id: order.id,
    type: 'order' as const,
    icon: Package,
  ...
  const attentionItems = [...(unassignedPickups > 0 ? [{
  ...
  return <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
        {kpiMetrics.map(metric => <Card key={metric.title} className={`cursor-pointer transition-all hover:shadow-md ${metric.alert ? 'border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-900/20' : 'border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800'}`} onClick={metric.onClick}>
            <CardContent className="p-4">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <metric.icon className={`h-4 w-4 ${metric.alert ? 'text-red-600' : 'text-gray-600 dark:text-gray-400'}`} />
                    <p className={`text-xs font-medium ${metric.alert ? 'text-red-700' : 'text-gray-600 dark:text-gray-400'}`}>
                      {metric.title}
                    </p>
                  </div>
                  <p className={`text-xl font-bold mt-1 ${metric.alert ? 'text-red-900 dark:text-red-300' : 'text-gray-900 dark:text-gray-100'}`}>
                    {metric.value}
                  </p>
                  <div className="flex items-center gap-1 mt-1">
                    {metric.trend.positive ? <ArrowUpRight className="h-3 w-3 text-green-600" /> : <ArrowDownRight className="h-3 w-3 text-red-600" />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent className="p-0">
            <div className="max-h-96 overflow-y-auto">
              {recentActivity.map((activity, index) => <div key={`${activity.type}-${activity.id}`} className="flex items-center gap-3 p-4 border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer transition-colors" onClick={() => navigate(activity.link)}>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {attentionItems.length === 0 ? <div className="text-center py-8">
                <CheckCircle className="h-12 w-12 text-green-600 mx-auto mb-2" />
                <p className="text-sm text-gray-600 dark:text-gray-400">
                  All systems running smoothly
                </p>
              </div> : attentionItems.map((item, index) => <div key={index} className={`p-3 rounded-lg border-l-4 ${item.type === 'error' ? 'border-red-500 bg-red-50 dark:bg-red-900/20' : 'border-orange-500 bg-orange-50 dark:bg-orange-900/20'}`}>
                  <div className="flex items-start gap-3">
                    <item.icon className={`h-4 w-4 mt-0.5 ${item.type === 'error' ? 'text-red-600' : 'text-orange-600'}`} />
                    <div className="flex-1">
                      <p className={`font-medium text-sm ${item.type === 'error' ? 'text-red-900' : 'text-orange-900'} dark:text-gray-100`}>
                        {item.title}
                      </p>
                      <p className={`text-xs mt-1 ${item.type === 'error' ? 'text-red-700' : 'text-orange-700'} dark:text-gray-300`}>
                        {item.description}
                      </p>
                      <Button size="sm" variant="outline" className="mt-2 h-7 text-xs" onClick={() => navigate(item.link)}>
                        {item.action}
                      </Button>
                    </div>
                  </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {highlights.map((highlight, index) => <div key={index} className="text-center p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
