...
interface PickupDetailsContentProps {
  pickup: PickupData;
}
...
export const PickupDetailsContent: React.FC<PickupDetailsContentProps> = ({ pickup }) => {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'In Progress': return <Package className="h-5 w-5" />;
      case 'Completed': return <Check className="h-5 w-5" />;
      case 'Canceled': return <X className="h-5 w-5" />;
  ...
  return (
    <div className="space-y-6 py-2 px-1 md:px-0">
      {/* Title */}
      <div className="text-xl flex items-center gap-2 font-bold">
        Pickup #{pickup.pickupId || pickup.id} {getStatusBadge(pickup.status)}
      </div>
      {/* Pickup Summary */}
            ...
            >
              <div className={cn(
                ...
                index <= currentStepIndex ?
                  getStatusColor(index === 2 && pickup.status === 'Canceled' ? 'Canceled' : step)
                  : "bg-gray-200"
              )}>
                {getStatusIcon(index === 2 && pickup.status === 'Canceled' ? 'Canceled' : step)}
              </div>
