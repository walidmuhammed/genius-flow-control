...
export async function createCourierSettlement(data: CreateSettlementData): Promise<CourierSettlement> {
  ...
  const balanceUSD = totalCourierFeesUSD - totalCollectedUSD;
  const balanceLBP = totalCourierFeesLBP - totalCollectedLBP;
  const direction = (balanceUSD >= 0 && balanceLBP >= 0) ? 'Owe Courier' : 'Courier Owes';
...
export async function getCourierBalance(courierId: string) {
  ...
  return {
    ...
    totalCourierFeesUSD,
    totalCourierFeesLBP,
    balanceUSD: totalCourierFeesUSD - totalCollectedUSD,
    balanceLBP: totalCourierFeesLBP - totalCollectedLBP,
...
// Request settlement payout (for courier use)
export async function requestSettlementPayout(courierId: string, orderIds: string[], notes?: string) {
  // Validate all orders belong to the courier and are ready for settlement
  ...
  if (!orders || orders.length !== orderIds.length) {
    throw new Error('Some orders are not eligible for settlement');
  }
  ...
  const settlement = await createCourierSettlement({
    ...
    order_ids: orderIds,
    notes: notes || 'Settlement requested by courier'
  });
