...
const Pickups: React.FC = () => {
  ...
  const upcomingPickups = pickupsData.filter(pickup => pickup.status === "Scheduled" || pickup.status === "In Progress");
  const historyPickups = pickupsData.filter(pickup => pickup.status === "Completed" || pickup.status === "Canceled");
          ...
          <Tabs defaultValue="upcoming" value={activeTab} onValueChange={setActiveTab} className="w-full">
            <div className="border-b">
              <TabsList className="bg-transparent h-12">
                <TabsTrigger value="upcoming" className="data-[state=active]:border-b-2 data-[state=active]:border-orange-500 data-[state=active]:shadow-none rounded-none h-12">
                  Upcoming Pickups
                </TabsTrigger>
                <TabsTrigger value="history" className="data-[state=active]:border-b-2 data-[state=active]:border-orange-500 data-[state=active]:shadow-none rounded-none h-12">
                  History
                </TabsTrigger>
              </TabsList>
            </div>
            ...
            <div className="p-4 flex items-center gap-4 border-b">
              <div className="relative flex-1">
                ...
                <Input placeholder="Search pickups by ID, location, or contact..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10" />
              </div>
            ...
            <CardContent className="p-0">
              <TabsContent value="upcoming" className="mt-0">
                {upcomingPickups.length === 0 ? <div className="flex flex-col items-center justify-center py-16">
                      ...
                      <TableBody>
                        {upcomingPickups.map(pickup => <TableRow key={pickup.id} className="hover:bg-gray-50 cursor-pointer">
                            ...
                            <TableCell className="text-center">
                              {pickup.requested ? <Badge className="bg-green-100 text-green-800 rounded-full px-2">Yes</Badge> : <Badge className="bg-gray-100 text-gray-800 rounded-full px-2">No</Badge>}
                            </TableCell>
                            <TableCell className="text-center">
                              {pickup.pickedUp ? <Badge className="bg-green-100 text-green-800 rounded-full px-2">Yes</Badge> : <Badge className="bg-gray-100 text-gray-800 rounded-full px-2">No</Badge>}
                            </TableCell>
                            <TableCell className="text-center">
                              {pickup.validated ? <Badge className="bg-green-100 text-green-800 rounded-full px-2">Yes</Badge> : <Badge className="bg-gray-100 text-gray-800 rounded-full px-2">No</Badge>}
                            </TableCell>
                          </TableRow>)}
                      </TableBody>
                    </Table>
              ...
              <TabsContent value="history" className="mt-0">
                <div className="overflow-x-auto">
                  <Table>
                    ...
                    <TableBody>
                      {historyPickups.map(pickup => <TableRow key={pickup.id} className="hover:bg-gray-50">
                          <TableCell>
                            <div className="flex items-center">
                              <span className="font-medium text-blue-600">{pickup.id}</span>
                              {pickup.note && <TooltipProvider>
